...
```{r, echo=FALSE}
aq = read.csv(file = "raw_data/Beijing_17/beijing_17_18_aq.csv", 
              header = TRUE, stringsAsFactors=FALSE)
meo = read.csv(file = "raw_data/Beijing_17/beijing_17_18_meo.csv", 
               header = TRUE, stringsAsFactors=FALSE)
stations = read.xlsx(xlsxFile = "raw_data/Beijing_17/Beijing_AirQuality_Stations_en.xlsx", 
                     colNames = TRUE)
...
```{r, echo = FALSE}
load("raw_data/Beijing_District/District.Rdata")
...
load(file = "raw_data/Beijing_District/District.Rdata")
...
if (file.exists("output/gp_jags.Rdata"))
{
  load(file="output/gp_jags.Rdata")
} else {
  m = rjags::jags.model(
  ...
  save(exp_cov_coda, file="output/gp_jags.Rdata")
}
```
...
```{r, echo = FALSE}
if (!file.exists("output/stan_car.rds")) {
  car_fit = rstan::stan(
    model_code = car_model, data = data,
    iter = 10000, chains = 1, thin=20
  )
  saveRDS(car_fit, "output/stan_car.rds")
} else {
  car_fit = readRDS("output/stan_car.rds")
}
```
...
```{r, echo = FALSE}
if (!file.exists("output/stan_sar.rds")) {
  sar_fit = rstan::stan(
    model_code = sar_model, data = data,
    iter = 10000, chains = 1, thin=20
  )
  saveRDS(sar_fit, "output/stan_sar.rds")
} else {
  sar_fit = readRDS("output/stan_sar.rds")
}
```
...
```{r, echo = FALSE}
if (!file.exists("output/car_lm_model.Rdata")) {
  m = rjags::jags.model(
  ...
  save(car_coda, m, file="output/car_lm_model.Rdata")
} else {
  load("output/car_lm_model.Rdata")
}
```
...
```{r, echo = FALSE}
if (!file.exists("output/car_lm_meo_model.Rdata")) {
  m = rjags::jags.model(
    textConnection(car_model), 
    data = list(
      D = D,
  ...
  save(car_coda, m, file="output/car_lm_meo_model.Rdata")
} else {
  load("output/car_lm_meo_model.Rdata")
}
```
...
```{r, echo=FALSE, warning = FALSE}
if (!file.exists("output/gplm.Rdata")) {
  m = jags.model(
    textConnection(gplm), 
    data = list(
  ...
  save(gplm_coda, file="output/gplm.Rdata")
} else {
  load("output/gplm.Rdata")
}
...
```{r, echo =FALSE, warning = FALSE}
if (!file.exists("output/splm.Rdata")) {
  ...
  save(m_pred_summary, file="output/splm.Rdata")
} else {
  load("output/splm.Rdata")
}
...
save(m, file = "output/sp_temp_mod.Rdata")
```

```{r, echo = FALSE}
load("output/sp_temp_mod.Rdata")
```
